...
class Pedal(nn.Module):
    ...
    def forward(self, feature, centers, position, PatchMemory = None, vid=None, camid=None, cfg=None):
        ...
        trick = torch.arange(dist_map.size(1)).cuda(cfg.MODEL.DEVICE).expand_as(dist_map)
        neg, index = dist_map[trick!=position.unsqueeze(dim=1).expand_as(dist_map)].view(dist_map.size(0), -1).sort(dim=1)
        
        pos_camid = torch.tensor(PatchMemory.camid).cuda(cfg.MODEL.DEVICE)
        pos_camid = pos_camid[(index[:,:self.k])]
        flag = pos_camid != camid.unsqueeze(dim=1).expand_as(pos_camid)
        
        pos_vid = torch.tensor(PatchMemory.vid).cuda(cfg.MODEL.DEVICE)
