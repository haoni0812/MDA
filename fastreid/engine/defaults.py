...
class DefaultTrainer(SimpleTrainer):
    def __init__(self, cfg):
        ...
        pda_model = vae_models['PDA'](2048,128,None,device=cfg.MODEL.DEVICE)
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TESTS):
            ...
            if 'ALL' in dataset_name:
                ...
                for x in sub_name:
                    ...
                    results_i = inference_on_dataset(model, data_loader, evaluator, cfg=cfg, model_opt=model_opt, use_adain=use_adain, pda_model=pda_model, pda_optimizer=pda_optimizer, pda_scheduler=pda_scheduler, opt=report_all)
                    if isinstance(x, int):
                        x = str(x)
                    if report_all:
                ...
                results_i = inference_on_dataset(model, data_loader, evaluator, cfg=cfg, model_opt=model_opt, use_adain=use_adain, pda_model=pda_model, pda_optimizer=pda_optimizer, pda_scheduler=pda_scheduler)
